import img1 from "../images/image.png";
import img2 from "../images/as.png";
import greenkart from "../images/greenkart.png";
import vidyanethrralogo from "../images/vidyanethrralogo.png";
import electrologo from "../images/electrologo.png";
import certificate from "../images/certificate.png";
import malware from "../images/malware.png";

export const Bio = {
  name: "Selva Sangeeth",
  roles: ["Full Stack Developer"],
  description:
    "I am a motivated and versatile individual, always excited to embrace new challenges. My passion for learning drives me to consistently deliver high-quality results. With a positive attitude and a commitment to growth, I am eager to contribute meaningfully and achieve success in every endeavor",
  github: "https://github.com/selvasangeeth/",
  resume:
    "https://drive.google.com/drive/folders/1803mEnBQPgHjpWWrJwyXQkbDgE0cINu_",
  linkedin: "https://www.linkedin.com/in/selva-sangeeth-s-950116238",
  insta: "https://www.instagram.com/its_sngth?igsh=MXRzZ29yeDYyNjg4",
  email:"mailto:selvasangeeth7@gmail.com"
};

export const skills = [
  {
    title: "Frontend",  
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      // {
      //   name: "Redux",
      //   image:
      //     "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      // },
      // {
      //   name: "Next Js",
      //   image:
      //     "data:image/png;base64,",
      // },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "https://img.icons8.com/?size=100&id=2ZOaTclOqD4q&format=png&color=000000",
      },
      {
        name: "MySQL",
        image:
          "https://img.icons8.com/?size=100&id=UFXRpPFebwa2&format=png&color=000000",
      },

      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },
      {
        name: "JWT",
        image:
          "https://img.icons8.com/?size=100&id=rHpveptSuwDz&format=png&color=000000",
      },
    ],
  },

  {
    title: "Others",
    skills: [
      {
         name:"Java",
         image: "https://img.icons8.com/?size=100&id=13679&format=png&color=000000",
      },
      {
        name: "Git",
        image:
          "https://img.icons8.com/?size=100&id=20906&format=png&color=000000",
      },
      {
        name: "GitHub",
        image:
          "https://img.icons8.com/?size=100&id=12599&format=png&color=000000",
      },

      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },

      {
        name: "Netlify",
        image:
          "https://seeklogo.com/images/N/netlify-logo-BD8F8A77E2-seeklogo.com.png",
      },

      {
        name: "Vercel",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Render",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Packet-Tracer",
        image:
          "https://img.icons8.com/?size=100&id=XEnbmdky0kzu&format=png&color=000000",
      },
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "https://img.icons8.com/?size=100&id=aczgoYL07gXY&format=png&color=000000",
    role: "Networking",
    company: "Thiran Hub",
    date: "Dec 2023",
    desc: `During my networking internship, I gained practical skills in configuring network devices, understanding protocols, and implementing security measures. Hands-on experience with routers and switches enhanced my ability to design and maintain efficient network infrastructures.`,
    skills: [
      "Subnetting",
      "VLAN",
      "Protocols ",
      "IPV4 AND IPV6",
      "Switch and routers",
      "DHCP"
    ],
    doc: "",
  },
  {
    id: 1,
    img: electrologo,
    role: "Mern Stack Internship",
    company: "ELECTRO CLOUD LABS",
    date: "June 2023 - July 2023",
    desc: "During my internship, I gained valuable hands-on experience in full-stack development, specializing in React.js, MongoDB, Express.js, and Node.js. This opportunity allowed me to deepen my knowledge of the MERN stack and develop expertise in building scalable and efficient web applications.",
    skills: [
      "React JS",
      "Express JS",
      "Node Js",
      "HTML",
      "CSS",
      "JavaScript",
      "MongoDB",
    ],
  },
];

export const education = [
  {
    id: 0,
    // img: "https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/Kiit.jpeg?alt=media&token=3839d520-c59d-4341-ad8f-0980c98de4dd",
    img: img2,
    school: "Sri Eshwar College of Engineering",
    date: "2021 - 2025",
    grade: "7.76 CGPA",
    desc: " ",
    degree: "BE., Computer and  Communication Engineering",
  },
  {
    id: 1,
    // img: "https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/methodist.png?alt=media&token=018a1b18-e2dd-4f34-8855-cff1b6b69fd3",
    img: vidyanethrralogo,
    school: "Vidya Nethrra Matriculation Hr Sec School",
    date: " 2020 - 2021",
    grade: "93.5%",
    // desc: "I completed my class 12 high school education at Methodist School, Dankuni, where I studied Science with Computer Science.",
    degree: "XII -  Science with Computer",
  },
];

export const projects = [
  {
    id: 9,
    title: "TASKLY",
    date: "2024",
    description:
      "Taskly is a robust task management platform designed to streamline the organization of daily activities. It enables users to seamlessly add and manage tasks while providing a clear, calendar-based visualization of deadlines and schedules. The intuitive calendar view enhances productivity by offering an at-a-glance overview of tasks and timelines, promoting better time management and efficient task tracking. Built with React.js, Node.js, Express.js, and MongoDB, Taskly ensures a smooth user experience with secure login features, utilizing JWT authentication.",
    image: "https://png.pngtree.com/thumb_back/fh260/background/20220427/pngtree-tiny-business-people-and-manager-at-tasks-and-goals-accomplishment-chart-image_1091427.jpg",
    tags: ["React Js", "MongoDB", "Node Js", "Express Js", "JWT "],
    category: "web app",
    github: "https://github.com/selvasangeeth/TaskManager",
    webapp: "https://tasklie.netlify.app/",
  },
  {
    id: 0,
    title: "GREEN KART",
    date: "2023",
    description:
      "An innovative e-commerce platform designed to empower farmers by providing easy access to agricultural products. Built with React, HTML, and CSS, it offers a seamless and user-friendly shopping experience. The platform enhances accessibility for buyers and sellers, promoting agricultural growth and convenience.",
    image: greenkart,

    tags: ["React Js", "HTML", "CSS"],
    category: "web app",
    github: "https://github.com/selvasangeeth/Green-Kart",
  },
  {
    id: 1,
    title: "CARE SYNC",
    date: "2023",
    description:
      "Care Sync is a secure medical management system allowing doctors to access patient histories through fingerprint authentication. Prioritizing streamlined record retrieval during emergencies, it was built using React js,Node js ,CSS, MongoDB, and C# for fingerprint devices. The platform enhances healthcare efficiency and showcases innovative solutions in medical management.",
    image:
      "https://img.freepik.com/premium-vector/group-emergency-room-doctors-nurses-medical-staff_1214-305.jpg",
    tags: [
      "React Js",
      "MongoDB",
      "Node Js",
      "Express Js",
      "C#",
      "JWT"
    ],
    category: "web app",
    github: "https://github.com/selvasangeeth/CareSync",
    // webapp: "https://vexa-app.netlify.app/",
  },

  // {
  //   id: 3,
  //   title: "FILELESS MALWARE ANALYSIS ",
  //   date: "2024",
  //   description:
  //     "The Fileless Malware Analysis project involves gathering RAM dump files and applying Machine Learning algorithms for advanced analysis. It integrates Bash Scrutiny to improve real-time detection and mitigation of evolving threats, providing an innovative approach to cybersecurity.",
  //   image:
  //     "https://camo.githubusercontent.",
  //   tags: ["Kotlin", "MVVM", "Room Database", "Google Drive Cloud API"],
  //   category: "android app",
  //   github: "",
  //   webapp: "",
  // },
  {
    id: 10,
    title: "  FILELESS MALWARE ANALYSIS   ",
    date: "2024",
    description:
      "The Fileless Malware Analysis project involves gathering RAM dump files and applying Machine Learning algorithms for advanced analysis. It integrates Bash Scrutiny to improve real-time detection and mitigation of evolving threats, providing an innovative approach to cybersecurity.",
    image: malware,
    // "https://user-images.githubusercontent.com/64485885/255237090-cf798a2c-1b41-4bb7-b904-b5353a1f08e8.png",
    tags: ["BashScript","Python", "Random-Forest","Numpy", "Dumpit-tool"],
    category: "android app",
    github: "https://github.com/selvasangeeth/Fileless_Malware_Analysis",
    webapp: "",
  },
  {
    id: 10,
    title: " LIMITED NETWORK BROADCAST ",
    date: "2024",
    description:
      "Created two different networks with each network of ten systems and configured the systems with IP address in Cisco Packet tracer. Implemented Subnetting and VLAN concepts. In normal switching the data will broadcast to all PCs but by using VLAN the broadcast is limited and sent to certain systems which ensures the security of the messages that travels through the network",
    image: "https://images.ctfassets.net/aoyx73g9h2pg/60RNKQ4jWkFRAYtyH3i3rW/7da3f4dc1358971754471cff294beca9/Subnetting-Network-Class-Blog.jpg?w=3840&q=100",
    // "https://user-images.githubusercontent.com/64485885/255237090-cf798a2c-1b41-4bb7-b904-b5353a1f08e8.png",
    tags: ["Switches", "Routers", "VLAN", "Subnetting", "Protocols"],
    category: "android app",
    github: "",
    webapp: "",
  },
];

export const TimeLineData = [
  { year: 2017, text: "Started my journey" },
  { year: 2018, text: "Worked as a freelance developer" },
  { year: 2019, text: "Founded JavaScript Mastery" },
  { year: 2020, text: "Shared my projects with the world" },
  { year: 2021, text: "Started my own platform" },
];
